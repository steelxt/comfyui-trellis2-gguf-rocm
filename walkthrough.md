...
13. **Sparse Sliced Attention & Naive Backend Support**: Resolved the High-Resolution Shape Slat Sampling (HR) Out-of-Memory (OOM) error on the RX 9060 XT (16GB VRAM) during GGUF generation.
    - We patched `config.py` in the sparse modules to recognize `"naive"` and `"sdpa"` backends.
    - We implemented a query-sliced attention mechanism `_sliced_sdpa` inside `full_attn.py` that processes large token sequences in blocks of `1024` tokens, bounding memory allocations.
    - We updated the `_sdpa_varlen` fallback function to use `_sliced_sdpa` directly when the backend is `"naive"`, and dynamically fall back to it if PyTorch's native `scaled_dot_product_attention` throws a HIP OOM under other backends.
    - We updated `windowed_attn.py` to correctly initialize window arguments and handle `"sdpa"` and `"naive"` options.
14. **Self-Healing Startup Patcher (Double-Patching Fix)**: Resolved a syntax error block (`unexpected character after line continuation character` and `IndentationError`) that occurred in `/app/ComfyUI/custom_nodes/ComfyUI-Trellis2-GGUF/__init__.py` when the container restarted. The build-time script had introduced literal `\n` sequence characters into the file, which the startup script matched recursively on subsequent container restarts, causing Python syntax corruption. We corrected the build-time patching syntax in `install-trellis2-gguf-rocm.sh`, and updated `entrypoint.sh` with a safeguard check (`if new in content: continue`) to prevent nested double-patching, restoring clean, warning-free container startup.
